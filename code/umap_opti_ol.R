...
c_m <- c_m <- as.matrix(read.delim("../results/data/overlap_matrix.txt", sep= " ", dec=".", header=T)) %>%
  .[-1,-1] #Exclude metaweb
...
ClusterOptimization <- function(Iterations, MinCluster, Unclustered) {
  ...
  top_c <- foreach(m=1:Iterations, .combine= rbind) %dopar% { #Each iteration tests all combination of UMAP scale and HDBSCAN cluster min size
    ...
    for(i in 2:Objects){ # i = UMAP scale, small numbers focus on more local scales
        ...
        umap_c <- umap(c_m, config = umap.mod, method = "umap-learn") # method requires installing the python package "umap-learn"
        ...
        allclust[count,3] <- c_melt[c_melt$value < 1,] %>% #Excludes comparing networks with themselves
        ...
        #Collapsing cluster lists and layouts (umap coordinates) instead of nested data frames
...
ol_topclusters <- ClusterOptimization(Iterations=it,
                                   MinCluster=clu,
                                   Unclustered=uncl) 
